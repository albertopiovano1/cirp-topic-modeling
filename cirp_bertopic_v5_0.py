...
Features:
- Rigorous data validation & quality reporting
- Dual text preprocessing (embedding-friendly + vectorizer-optimized)
- Controlled synonym normalization (safe + optional aggressive modes)
- Publication-ready metrics (Diversity, Coherence C_v, Stability)
- Time-series analysis with cutoff control
- Comprehensive CSV exports for qualitative analysis
...
class Config:
    ...
    # Text preprocessing
    MIN_WORDS_PER_DOC = 10  # Minimum words after cleaning
    MIN_ABSTRACT_LENGTH = 50  # Minimum abstract length in words
    ...
    # Analysis parameters
    TIME_CUTOFF_YEAR = 2024  # Exclude data after this year for trends
    TOP_N_TOPICS_VISUALIZATION = 16
    TOP_N_PAPERS_PER_TOPIC = 5  # For qualitative sampling
    
    # Stability bootstrap
    STABILITY_N_SAMPLES = 1.0  # Fraction of data for bootstrap
...
class TextPreprocessor:
    ...
    - text_embed: Natural language for embeddings (conservative cleaning)
    - text_vec: Processed text for c-TF-IDF vectorizer (aggressive cleaning)
    """
    ...
    def _setup_synonym_maps(self):
        ...
        self.safe_synonyms = {
            ...
            "print": "additive_manufacturing",  # In manufacturing context
    ...
    def _setup_stopwords(self):
        ...
        # Generic manufacturing terms (too broad)
        stopwords_domain = [
            "food", "sector", "industry", "product", "system", "solution",
            "process", "production", "company", "business"
        ]
    ...
    def clean_text_for_embedding(self, text: str) -> str:
        """
        Conservative cleaning for embedding model (maintains natural language)
        """
    ...
    def clean_text_for_vectorizer(self, text: str) -> str:
        """
        Aggressive cleaning for c-TF-IDF vectorizer (optimized for keyword extraction)
        """
        if not isinstance(text, str) or not text.strip():
            return ""
    ...
    def lemmatize_and_normalize(self, text: str, for_vectorizer: bool = False) -> str:
        ...
        Args:
            text: Input text
            for_vectorizer: If True, apply underscore joining for compounds
        """
        doc = self.nlp(text)
        tokens = []
        ...
        # For vectorizer: join compounds with underscore
        if for_vectorizer:
            return " ".join(tokens)
        else:
            # For embeddings: keep natural spacing
            return " ".join(tokens)
    ...
    def process_corpus(self, 
        ...
        Returns:
            text_embed: List of texts for embedding (natural language)
            text_vec: List of texts for vectorizer (underscore compounds)
...
class BERTopicModelBuilder:
    ...
    def build_model(self) -> BERTopic:
        """Build BERTopic model with publication-ready configuration"""
...
# ============================================================================
# CONTINUED IN NEXT FILE (cirp_bertopic_v5_0_part2.py)
# ============================================================================

# ============================================================================
# 5. MODEL TRAINING & OUTLIER REDUCTION
# ============================================================================

class ModelTrainer:
    """Handle model training and outlier reduction"""
    ...
    def train(self, texts: List[str], embeddings: np.ndarray) -> Tuple[List[int], np.ndarray]:
        ...
        Args:
            texts: Preprocessed texts (text_vec for c-TF-IDF)
            embeddings: Pre-computed embeddings (from text_embed)
    ...
    def reduce_outliers(self, texts: List[str], embeddings: np.ndarray):
        """Reduce outliers using embedding strategy"""
...
class MetricsCalculator:
    """Calculate publication-ready quality metrics"""
    
    def __init__(self, model: BERTopic, texts: List[str], embedding_model):
        self.model = model
        self.texts = texts
    ...
    def calculate_all_metrics(self) -> Dict:
        ...
        print("\n[3/3] Calculating Stability (Bootstrap)...")
    ...
    def _calculate_coherence(self, 
        ...
        """Calculate coherence C_v score using Gensim"""
        try:
            cm = CoherenceModel(
                topics=top_words,
    ...
    def _calculate_stability(self) -> float:
        """Calculate topic stability via bootstrap resampling"""
        try:
            # Bootstrap sample
            ...
            boot_embeddings = self.embedding_model.encode(
                boot_docs, 
            ...
            if orig_centroids and boot_centroids:
                for tid, orig_vec in orig_centroids.items():
                    # Find best match in bootstrap model
                    best_sim = 0
    ...
    def _get_topic_centroids(self, model: BERTopic) -> Dict[int, np.ndarray]:
        """Extract topic centroids as embedding vectors"""
        centroids = {}
        for tid in model.get_topics().keys():
            ...
            if words:
                # Use embedding of concatenated top words as centroid
                centroid_text = " ".join(words)
                centroid_vec = self.embedding_model.encode(
                    centroid_text, 
                    show_progress_bar=False
                )
...
class DataExporter:
    ...
    def export_all(self):
        ...
        # 2. Top papers per topic (qualitative sampling)
        self._export_top_papers(df_aligned)
        
        # 3. Topic metadata and keywords
    ...
    def _export_top_papers(self, df: pd.DataFrame):
        """Export most relevant papers per topic for qualitative review"""
        ...
        for tid in valid_topics:
            ...
            # Sort by probability (best proxy for relevance)
...
class VisualizationGenerator:
    """Generate BERTopic visualizations with time cutoff"""
    ...
    def _generate_topics_over_time(self):
        """Topics over time with year cutoff"""
...
def main():
    ...
    print("\n" + "="*80)
    print("CIRP BIBLIOMETRIC ANALYSIS - BERTopic Pipeline v5.0")
    print("="*80)
    ...
    # 4. Generate embeddings (from text_embed)
    print("\n" + "="*80)
    ...
    # 5. Train model (using text_vec for c-TF-IDF)
    ...
    # 7. Calculate metrics
    metrics_calc = MetricsCalculator(model, text_vec, model.embedding_model)
...
def _save_final_report(validation_report: Dict, metrics: Dict, topics: List[int]):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"Pipeline Version: v5.0\n")
        ...
        f.write(f"UMAP: n_neighbors={Config.UMAP_N_NEIGHBORS}, ")
        f.write(f"n_components={Config.UMAP_N_COMPONENTS}\n")
        f.write(f"HDBSCAN: min_cluster_size={Config.HDBSCAN_MIN_CLUSTER_SIZE}, ")
        f.write(f"min_samples={Config.HDBSCAN_MIN_SAMPLES}\n")
        f.write(f"Vectorizer: ngram_range={Config.VECTORIZER_NGRAM_RANGE}, ")
        f.write(f"min_df={Config.VECTORIZER_MIN_DF}\n\n")
