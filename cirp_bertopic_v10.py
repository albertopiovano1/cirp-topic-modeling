...
"""
================================================================================
CIRP BIBLIOMETRIC ANALYSIS - BERTopic Pipeline v6.8 (SPECTER2-inspired embeddings + knowledge-extent metrics) (natural, metrics-aligned: ngram coherence + MMR rep)
================================================================================
...
# Optional: SPECTER2 (AllenAI) encoder (as used in AI-Impacts-Science)
try:
    import torch
...
class Config:
    ...
    AUTO_STOPWORD_TOP_N: int = 0                  # add top-N most frequent tokens in corpus (after lemmatization)
    ...
    # --- Embeddings ---
    # Backend: "specter2" (recommended for scientific corpora) or "sentence_transformer"
    EMBEDDING_BACKEND: str = "specter2"

    # SPECTER2 settings (only used if EMBEDDING_BACKEND == "specter2")
    SPECTER2_BASE_MODEL: str = "allenai/specter2_base"
    SPECTER2_ADAPTER: str = "allenai/specter2"
    SPECTER2_ADAPTER_NAME: str = "specter2"
    SPECTER2_MAX_LENGTH: int = 512

    # Sentence-Transformers fallback (used if EMBEDDING_BACKEND == "sentence_transformer")
    EMBEDDING_MODEL: str = "BAAI/bge-large-en-v1.5"
    EMBEDDING_BATCH_SIZE: int = 32
    USE_EMBEDDING_CACHE: bool = True
    CACHE_TAG: str = "v6_7"   # bump this if you change preprocessing in a way that invalidates cached embeddings
    ...
    VECTORIZER_NGRAM_RANGE: Tuple[int, int] = (1, 3)
    VECTORIZER_MIN_DF: int = 10
    VECTORIZER_MAX_FEATURES: int = 20000
...
class Specter2Encoder:
    """Minimal SentenceTransformer-like wrapper around AllenAI SPECTER2.

    Provides an `.encode(...)` method compatible with BERTopic/KeyBERT-inspired representations.
    """
    def __init__(self, device: Optional[str] = None):
        if not _HAS_SPECTER2:
            raise ImportError(
                "SPECTER2 backend requested but required packages are missing. "
                "Install with: pip install adapters transformers torch"
            )
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = AutoTokenizer.from_pretrained(Config.SPECTER2_BASE_MODEL)
        self.model = AutoAdapterModel.from_pretrained(Config.SPECTER2_BASE_MODEL)
        # Load the SPECTER2 adapter and activate it
        self.model.load_adapter(
            Config.SPECTER2_ADAPTER,
            source="hf",
            load_as=Config.SPECTER2_ADAPTER_NAME,
            set_active=True,
        )
    ...
    def _mean_pool(last_hidden_state: "torch.Tensor", attention_mask: "torch.Tensor") -> "torch.Tensor":
        # Not used by default; kept for experimentation
    ...
    def encode(
        ...
        if show_progress_bar:
            it = tqdm(list(it), desc="Batches", leave=False)
        ...
        with torch.no_grad():
            for start in it:
                ...
                inputs = self.tokenizer(
                    ...
                    padding=True,
                    truncation=True,
                    max_length=Config.SPECTER2_MAX_LENGTH,
                    return_tensors="pt",
                )
                inputs = {k: v.to(self.device) for k, v in inputs.items()}
                outputs = self.model(**inputs)
                # CLS token representation (SPECTER-style)
...
class EmbeddingGenerator:
    def __init__(self, results_dir: Path):
        ...
        if Config.EMBEDDING_BACKEND.lower() == "specter2":
    ...
    def _cache_path(self, n_docs: int) -> Path:
        ...
        backend = safe_slug(Config.EMBEDDING_BACKEND)
        if Config.EMBEDDING_BACKEND.lower() == "specter2":
            model = safe_slug(Config.SPECTER2_ADAPTER_NAME)
        else:
            model = safe_slug(Config.EMBEDDING_MODEL.split("/")[-1])
        return cache_dir / f"emb_{backend}_{model}_{tag}_{n_docs}.npy"
...
def build_model(encoder, vectorizer_stopwords: Optional[List[str]] = None) -> BERTopic:
    print("\n" + "=" * 80)
    print("BERTOPIC MODEL CONFIGURATION")
    print("=" * 80)
    if Config.EMBEDDING_BACKEND.lower() == "specter2":
        print(f"[1/6] Embedding model: SPECTER2 ({Config.SPECTER2_BASE_MODEL} + {Config.SPECTER2_ADAPTER}) (pre-encoded)")
    else:
    ...
    if _HAS_MMR and MaximalMarginalRelevance is not None:
        representation_model = [KeyBERTInspired(), MaximalMarginalRelevance(diversity=0.30)]
        print("    • Models: KeyBERTInspired + MaximalMarginalRelevance(0.30)")
    else:
...
class MetricsCalculator:
    ...
    def calculate_all(self) -> Dict[str, float]:
        ...
        vectorizer = getattr(self.model, "vectorizer_model", None)
        if vectorizer is not None and hasattr(vectorizer, "build_analyzer"):
            ana = vectorizer.build_analyzer()
            tokenized_docs = [ana(doc) for doc in self.texts]
        else:
...
def topics_over_time_table(df_assign: pd.DataFrame, results_dir: Path) -> pd.DataFrame:
    """Export raw Topics-over-Time tables for downstream plotting (e.g., Streamgraph/Joyplot in R).

    Outputs:
      - 06_Topics_Over_Time.csv               : long table (only non-zero Year×Topic)
      - 06a_Topics_Over_Time_FullGrid.csv     : long table with explicit zeros for missing Year×Topic
      - 06b_Topics_Over_Time_WideCount.csv    : wide table (rows=Year, cols=T<topic_id>) with raw counts
      - 06c_Topics_Over_Time_WideShare.csv    : wide table (rows=Year, cols=T<topic_id>) with shares

    Notes:
      - By default includes Topic=-1 (outliers) if Config.INCLUDE_OUTLIERS_IN_TIMEPLOTS is True.
      - Years are clipped to [2000, Config.TIME_CUTOFF_YEAR].
    """
    df = df_assign.copy()
    ...
    # Optionally drop outliers from time tables
    if not Config.INCLUDE_OUTLIERS_IN_TIMEPLOTS:
        df = df[df["Topic"] != -1].copy()

    # Ensure Topic_Name exists (fallback for outliers)
    if "Topic_Name" not in df.columns:
        df["Topic_Name"] = df["Topic"].astype(str)
    df.loc[df["Topic"] == -1, "Topic_Name"] = df.loc[df["Topic"] == -1, "Topic_Name"].replace({"-1": "Outliers"})
    df["Topic_Name"] = df["Topic_Name"].fillna("")

    # Aggregate (non-zero combos)
    ...
    # ----- Full grid (explicit zeros) for R streamgraphs/joyplots -----
    year_min = int(df["Year"].min()) if len(df) else 2000
    year_max = int(df["Year"].max()) if len(df) else Config.TIME_CUTOFF_YEAR
    years = np.arange(year_min, year_max + 1, dtype=int)

    # Map Topic -> Topic_Name (stable)
    topic_map = (
        df[["Topic", "Topic_Name"]]
    ...
    if -1 in topic_map["Topic"].values:
        topic_map.loc[topic_map["Topic"] == -1, "Topic_Name"] = "Outliers"

    topics = topic_map["Topic"].astype(int).tolist()

    grid = pd.MultiIndex.from_product([years, topics], names=["Year", "Topic"]).to_frame(index=False)
    grid = grid.merge(topic_map, on="Topic", how="left")

    full = grid.merge(out, on=["Year", "Topic", "Topic_Name"], how="left")
    full = full.merge(totals, on="Year", how="left", suffixes=("", "_y"))

    # If merge created duplicate Total columns, harmonize
    if "Total_y" in full.columns:
        full["Total"] = full["Total"].fillna(full["Total_y"])
        full = full.drop(columns=["Total_y"])

    full["Count"] = full["Count"].fillna(0).astype(int)
    full["Total"] = full["Total"].fillna(0).astype(int)
    full["Share"] = np.where(full["Total"] > 0, full["Count"] / full["Total"], 0.0)

    # Add a rank (useful for consistent ordering in R)
    topic_rank = full.groupby(["Topic", "Topic_Name"])["Count"].sum().reset_index()
    topic_rank = topic_rank.sort_values("Count", ascending=False).reset_index(drop=True)
    topic_rank["Topic_Rank"] = np.arange(1, len(topic_rank) + 1)
    full = full.merge(topic_rank[["Topic", "Topic_Rank"]], on="Topic", how="left")

    full_path = results_dir / "06a_Topics_Over_Time_FullGrid.csv"
    full.to_csv(full_path, index=False)
    print(f"✅ Saved topics over time (full grid): {full_path}")

    # Wide exports (counts + shares) for quick use in R
    wide_count = full.pivot_table(index="Year", columns="Topic", values="Count", fill_value=0)
    wide_count.columns = [f"T{int(c)}" for c in wide_count.columns]
    wide_count = wide_count.sort_index()
    # Attach totals
    totals_idx = totals.set_index("Year")["Total"]
    wide_count.insert(0, "Total", totals_idx.reindex(wide_count.index).fillna(0).astype(int).values)
    wide_count_path = results_dir / "06b_Topics_Over_Time_WideCount.csv"
    wide_count.to_csv(wide_count_path)
    print(f"✅ Saved topics over time (wide counts): {wide_count_path}")

    wide_share = full.pivot_table(index="Year", columns="Topic", values="Share", fill_value=0.0)
    wide_share.columns = [f"T{int(c)}" for c in wide_share.columns]
    wide_share = wide_share.sort_index()
    wide_share_path = results_dir / "06c_Topics_Over_Time_WideShare.csv"
    wide_share.to_csv(wide_share_path)
    print(f"✅ Saved topics over time (wide shares): {wide_share_path}")

    return out



def knowledge_extent_over_time(df_assign: pd.DataFrame, embeddings: np.ndarray, results_dir: Path) -> Optional[pd.DataFrame]:
    """Compute embedding-space 'knowledge extent' (diameter from centroid) and topic entropy over time.

    Inspired by the AI-Impacts-Science / Nature study methodology:
    - Compute centroid of embeddings for a set of papers.
    - Define 'extent' as the maximum Euclidean distance from centroid (a diameter proxy).
    - Add entropy of the topic distribution as a complementary concentration metric.
    """
    if "Year" not in df_assign.columns:
        return None
    if len(df_assign) != embeddings.shape[0]:
        print("⚠️  Skipping knowledge-extent: df_assign and embeddings have inconsistent lengths.")
        return None

    rows = []
    for year, idxs in df_assign.groupby("Year").indices.items():
        idxs = list(idxs)
        E = embeddings[idxs]
        if E.shape[0] < 5:
            continue
        ...
        extent = float(dist.max())

        # Topic entropy (excluding -1 by default)
        t = df_assign.loc[idxs, "Topic"].values
        t_non = t[t != -1]
        if len(t_non) == 0:
            entropy = 0.0
            entropy_norm = 0.0
            ...
            vals, counts = np.unique(t_non, return_counts=True)
            p = counts / counts.sum()
            entropy = float(-(p * np.log(p + 1e-12)).sum())
            k = len(vals)
            entropy_norm = float(entropy / np.log(k)) if k > 1 else 0.0

        out_share = float((t == -1).mean())

        rows.append({
            "Year": int(year),
            "N_docs": int(len(idxs)),
            "Outlier_Share": out_share,
            "Embedding_Extent": extent,
            "Topic_Entropy": entropy,
            "Topic_Entropy_Normalized": entropy_norm,
            "N_topics_non_outlier": int(k),
        })

    out = pd.DataFrame(rows).sort_values("Year")
    out_path = results_dir / "08_Knowledge_Extent_Over_Time.csv"
    out.to_csv(out_path, index=False)
    print(f"✅ Saved knowledge-extent over time: {out_path}")
    return out
...
def write_final_report(results_dir: Path, figures_dir: Path, n_docs: int, n_topics: int, n_out: int, metrics: Dict[str, float]) -> None:
    ...
    with out.open("w", encoding="utf-8") as f:
        ...
        f.write("Pipeline Version: v6.8 (natural, SPECTER2-inspired embeddings + knowledge-extent metrics)\n")
...
def main():
    print("\n" + "=" * 80)
    print("CIRP BIBLIOMETRIC ANALYSIS - BERTopic Pipeline v6.8 (SPECTER2-inspired embeddings + knowledge-extent metrics)")
    print("=" * 80)
    ...
    # Embedding-space metrics (extent + entropy), inspired by AI-Impacts-Science
    try:
        knowledge_extent_over_time(df_assign, embeddings, results_dir)
    except Exception as e:
        print(f"⚠️  Error computing knowledge-extent metrics: {e}")
