"""
================================================================================
CIRP BIBLIOMETRIC ANALYSIS - BERTopic Pipeline v5.1 (paper-ready)
================================================================================
...
- Fewer, more interpretable topics (stronger clustering + optional topic reduction)
- Metrics: diversity + coherence + FIXED stability (bootstrap on topic centroids)
- Controlled outlier reassignment (never forces 0% outliers by default)
================================================================================
"""
...
class Config:
    ...
    # Optional topic reduction (post-hoc merging)
    ENABLE_TOPIC_REDUCTION: bool = True
    TARGET_NR_TOPICS: int = 60           # Typical paper-ready range: 40–80

    # Outlier handling
    ENABLE_OUTLIER_REASSIGN: bool = True
    OUTLIER_REASSIGN_STRATEGY: str = "embeddings"   # "embeddings" usually best for scientific corpora
    OUTLIER_REASSIGN_THRESHOLD: float = 0.15        # keep some true outliers; avoid forced 0%
...
class ModelTrainer:
    ...
    def reduce_topics(self, texts_vec: List[str]) -> List[int]:
        if not Config.ENABLE_TOPIC_REDUCTION:
            return self.topics

        if self.topics is None:
            raise RuntimeError("Train the model before reducing topics.")

        print(f"\n🧩 Reducing topics to ~{Config.TARGET_NR_TOPICS} (merging micro-topics)...")
        try:
            # BERTopic API has varied across versions → handle both return styles
            out = self.model.reduce_topics(texts_vec, nr_topics=Config.TARGET_NR_TOPICS)
            ...
            print(f"   ✅ Reduced to {n_topics} topics; outliers: {n_outliers:,} ({n_outliers/len(new_topics)*100:.1f}%)")
            self.topics = new_topics
            return new_topics
        except Exception as e:
            print(f"   ⚠️  Error reducing topics: {e}")
            return self.topics
...
class VisualizationGenerator:
    ...
    def _generate_topics_over_time(self):
        if "Year" not in self.df_aligned.columns:
            ...
            return
        try:
            timestamps = pd.to_datetime(self.df_aligned["Year"].astype(str), format="%Y", errors="coerce")
            ...
            tot = tot[tot["Timestamp"].dt.year <= Config.TIME_CUTOFF_YEAR]
            out_path = Config.FIGURES_DIR / f"Viz_05_Topics_Over_Time_to_{Config.TIME_CUTOFF_YEAR}.html"
            fig = self.model.visualize_topics_over_time(tot, top_n_topics=12)
            fig.write_html(str(out_path))
            print(f"✅ Saved: {out_path}")
        except Exception as e:
            print(f"⚠️  Error generating time series: {e}")
...
def main():
    print("\n" + "=" * 80)
    print("CIRP BIBLIOMETRIC ANALYSIS - BERTopic Pipeline v5.1")
    print("=" * 80)
    ...
    # Optional merging to paper-ready number of topics
    topics = trainer.reduce_topics(text_vec)
...
def _save_final_report(validation_report: Dict, metrics: Dict, topics: List[int]):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Topic reduction: {Config.ENABLE_TOPIC_REDUCTION} (target={Config.TARGET_NR_TOPICS})\n")
